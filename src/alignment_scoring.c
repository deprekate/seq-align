...
void scoring_system_BLOSUM62(scoring_t* scoring)
{
  ...
  // no_start_gap_penalty, no_end_gap_penalty = 0
  // case_sensitive = 0
  scoring_init(scoring, 1, -4, -11, -1, 0, 0, 0, 0, 0, 0);

  // use_match_mismatch=1
  scoring_add_mutations(scoring, amino_acids, blosum62, 1);
}

// Scoring for protein comparisons of length 
void scoring_system_BLOSUM50(scoring_t* scoring)
{
  // *->* match: 1
  // *->* mismatch: -4
  // Gap open -10, gap extend -2
  // no_start_gap_penalty, no_end_gap_penalty = 0
  // case_sensitive = 0
  scoring_init(scoring, 1, -5, -10, -2, 0, 0, 0, 0, 0, 0);

  // use_match_mismatch=1
  scoring_add_mutations(scoring, amino_acids, blosum50, 1);
}
